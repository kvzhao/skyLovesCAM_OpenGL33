...
MillerRender::MillerRender()
{
   ...
   //  gOrientation1;
     ...
     window = glfwCreateWindow( 1024, 768, "Antweak and Loading model test", NULL, NULL);
     if( window == NULL ){
     ...
     TwBar * EulerGUI2 = TwNewBar("Workpiece settings");
    //     TwBar * QuaternionGUI = TwNewBar("Quaternion settings");
     TwSetParam(EulerGUI, NULL, "refresh", TW_PARAM_CSTRING, 1, "0.1");
       TwSetParam(EulerGUI2, NULL, "position", TW_PARAM_CSTRING, 1, "808 16");
     ...
     TwAddVarRW(EulerGUI2, "Euler X", TW_TYPE_FLOAT, &gOrientation2.x, "step=0.5");
     TwAddVarRW(EulerGUI2, "Euler Y", TW_TYPE_FLOAT, &gOrientation2.y, "step=0.5");
     TwAddVarRW(EulerGUI2, "Euler Z", TW_TYPE_FLOAT, &gOrientation2.z, "step=0.5");
     TwAddVarRW(EulerGUI2, "Pos X"  , TW_TYPE_FLOAT, &gPosition2.x, "step=0.5");
     TwAddVarRW(EulerGUI2, "Pos Y"  , TW_TYPE_FLOAT, &gPosition2.y, "step=0.5");
     TwAddVarRW(EulerGUI2, "Pos Z"  , TW_TYPE_FLOAT, &gPosition2.z, "step=0.5");
    ...
    // char* file = "/home/kaeon/MyProgram/OpenGL-33-myproject/src/cube.obj";
     ...
     programID = LoadShaders(
                 "../src/StandardShading.vertexshader",
     ...
     // calculate indices
     //loadOBJ("../src/ES4.STL", indices2, indexed_vertices2, indexed_uvs2,indexed_normals2);
     loadOBJ("../src/cube.obj", indices2,indexed_vertices2,indexed_uvs2,indexed_normals2);
...
//=============================================================================//
         ...
         /*    gOrientation1.z += 3.14159f/2.0f * deltaTime * 5;
             ...
             glm::mat4 ScalingMatrix = scale(mat4(), vec3(1.0f, 1.0f, 1.0f));
             glm::mat4 ModelMatrix = TranslationMatrix * RotationMatrix * ScalingMatrix;*/
             //gOrientation2.z += 3.14159f/2.0f * deltaTime/1000.0;
             //gOrientation2.x = 3.14159f/2;
             //gPosition2.y = 40;
             //tt = tt + 0.01f;
             //gPosition2.x = 20.0*sin(tt/100.0);
             //gPosition1.z = tt;//20.0*sin(tt);
             ...
             glDrawElements(
                         ...
                         indices2.size(),    // count
                         GL_UNSIGNED_SHORT,   // type
                         (void*)(0 + indices.size()*sizeof(unsigned short))           // element array buffer offset
                         );
